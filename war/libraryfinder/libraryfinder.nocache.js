...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "libraryfinder" because\n';
  ...
  $wnd.__gwt_activeModules['libraryfinder'] = {
    'moduleName' : 'libraryfinder',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/libraryfinder.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['libraryfinder'].superdevmode = true;

  var expectedSuffix = '/libraryfinder.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + libraryfinder + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:libraryfinder';
  var devModeSessionKey = '__gwtDevModeSession:libraryfinder';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/libraryfinder/libraryfinder.nocache.js';
  ...
  $wnd.__gwt_activeModules['libraryfinder'].canRedirect = true;
  $wnd.__gwt_activeModules['libraryfinder'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING libraryfinder';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/libraryfinder?user.agent=' + ua + '&_callback=' + callback;
